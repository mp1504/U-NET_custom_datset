...
class myUnet(object):
  ...
  def test(self, checkpoint_dir, _batch_size):
    ...
    self.model.load_weights(checkpoint_dir + '/unet.hdf5')
    steps, processed = math.ceil(test_generator.samples / _batch_size), 0
    piclist = [i.split(os.sep)[-1] for i in test_generator.filepaths]
    (width, height) = Image.open('./data/test/images/' + piclist[0]).size
    for _ in range(steps):
      batch = test_generator.batch_index
      imgs_predicted = self.model.predict_on_batch(next(test_generator))
      for i in range(imgs_predicted.shape[0]):
        path = "./data/results/" + piclist[batch * _batch_size + i]
        array_to_img(imgs_predicted[i]).resize((width,height)).save(path)
        processed += 1
      if processed % (_batch_size * 5) == 0:
